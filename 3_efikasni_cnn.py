...
if __name__ == "__main__":
    ...
    # --- 5. CELINA: MODEL #1 - EFIKASNI CNN ---
    # (Ovo ide umesto tvoje definicije za model1 i model2)

    from tensorflow.keras.layers import SeparableConv1D, GlobalAveragePooling1D

    print("\n=== MODEL #1: Efikasni CNN (MobileConv1D) ===")
    model_efficient = Sequential([
        Input(shape=(MAX_LEN, N_MFCC)),
        
        # Prvi blok
        SeparableConv1D(32, 3, activation='relu', padding='same'),
        BatchNormalization(),
        MaxPooling1D(2),
        Dropout(0.2),
        
        # Drugi blok
        SeparableConv1D(64, 3, activation='relu', padding='same'),
        BatchNormalization(),
        MaxPooling1D(2),
        Dropout(0.2),
        
        # Treći blok
        SeparableConv1D(128, 3, activation='relu', padding='same'),
        BatchNormalization(),
        MaxPooling1D(2),
        Dropout(0.2),
        
        # Ključni trik: Globalno pool-ovanje umesto Flatten + Dense
        GlobalAveragePooling1D(),
        
        # Finalni klasifikator
        Dense(64, activation='relu'),
        Dropout(0.3),
        Dense(NUM_CLASSES, activation='softmax')
    ])

    model = model_efficient # Izaberi ovaj model
    model.summary()

    # ... nastavi na CelinU 6 (Kompajliranje) ...
